...
echo "[INFO] Checking version information."
case "X$VERSION" in
  X|XLATEST|Xlatest)
...
function buildSpigotFromSource {
  echo "[INFO] Building Spigot $VANILLA_VERSION from source, might take a while, get some coffee"
  mkdir /data/temp
  cd /data/temp
...
echo "[INFO] Using $TYPE"

# If supplied with a URL for a world, download it and unpack
if [[ "$WORLD" ]]; then
case "X$WORLD" in
  X[Hh][Tt][Tt][Pp]*)
    echo "[INFO] Downloading world via HTTP"
    echo "[INFO] $WORLD"
    wget -q -O - "$WORLD" > /data/world.zip
    echo "[INFO] Unzipping word"
    unzip -q /data/world.zip
    rm -f /data/world.zip
    if [ ! -d /data/world ]; then
      echo [INFO] World directory not found
...
if [ ! -e server.properties ]; then
  echo "[INFO]Creating server.properties"
...
then
    ...
    echo "[INFO] Starting server: java $JMV_OPTS -jar $SERVER $@"
